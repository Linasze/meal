<div id="headingTwo">
   ...
   <div class="col-lg-2 d-none d-lg-block text-center" ><?php if(substr(date('h',strtotime($data['user_settings']->wake_up))+3, 0, 1 ) != 1){echo "0";}?><?php echo date('h',strtotime($data['user_settings']->wake_up)) + 3 .":00";?><br>Brunch</div>
     ...
     <img src="<?php echo URLROOT;?>/public/img/arrow-down.svg" height="15" weight="15">
       <font size="2"><?php echo $data['brunch3Day']->title; ?></font>
     </button>
     </div>
...
<div class="col-12">Brunch<br>Time: <?php if(substr(date('h',strtotime($data['user_settings']->wake_up))+3, 0, 1 ) != 1){echo "0";}?><?php echo date('h',strtotime($data['user_settings']->wake_up)) + 3 .":00";?></div>
    <div class="col-12"><?php echo $data['brunch3Day']->title; ?></div>
...
<div class="col-12">Recipe: <input type="image" src="<?php echo URLROOT;?>/public/img/eye-solid.svg" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="true" aria-controls="collapseTwo" height="25" width="25"></div>
     ...
     <div class="col-md-12 d-none d-md-block d-lg-none">
        ...
        <div class="col-md-12"><?php echo $data['brunch3Day']->title; ?></div>
        <hr>
...
<?php if($product->id == $data['proteinBrunch3day'][$i]): ?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['proteinBrunch3day'][$i]){
            echo round($data['proteinsperservingUseEmptybrunch']/2 * 100/ $product->protein).  " g". "<br>";
        }
    }else{

        if(count($data['proteinBrunch3day']) == 2 && empty($data['get_product1brunch']->use_id)){
            if($product->id == $data['proteinBrunch3day'][$i]){
                echo round($data['proteinsperservingUseEmptybrunch']/2 * 100/ $product->protein).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['proteinBrunch3day']) == 2 && !empty($data['get_product1brunch']->use_id)){
            if($product->id == $data['proteinBrunch3day'][$i]){
                echo round($data['proteinsperservingUseEmptybrunch'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinBrunch3day']) == 3 && !empty($data['get_product1brunch']->use_id) && !empty($data['get_product2brunch']->use_id)){
            if($product->id == $data['proteinBrunch3day'][$i]){
                echo round($data['proteinsperservingUseEmptybrunch'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinBrunch3day']) == 3 && !empty($data['get_product2brunch']->use_id)){
            if($product->id == $data['proteinBrunch3day'][$i]){
                echo round($data['proteinsperservingUseEmptybrunch']/2 * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinBrunch3day']) == 4 && !empty($data['get_product3brunch']->use_id)){
            if($product->id == $data['proteinBrunch3day'][$i]){
                echo round($data['proteinsperservingUseEmptybrunch'] * 100/ $product->protein).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['carbBrunch3day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['carbBrunch3day'][$i]){
            echo round($data['carbsperservingUseEmptybrunch']/2 * 100/ $product->carb).  " g". "<br>";
        }
    }else{

        if(count($data['carbBrunch3day']) == 2 && empty($data['getProduct1brunch']->use_id)){
            if($product->id == $data['carbBrunch3day'][$i]){
                echo round($data['carbsperservingUseEmptybrunch']  * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['carbBrunch3day']) == 2 && !empty($data['getProduct1brunch']->use_id)){
            if($product->id == $data['carbBrunch3day'][$i]){
                echo round($data['carbsperservingUseEmptybrunch'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbBrunch3day']) == 3 && !empty($data['getProduct1brunch']->use_id) && !empty($data['getProduct2brunch']->use_id)){
            if($product->id == $data['carbBrunch3day'][$i]){
                echo round($data['carbsperservingUseEmptybrunch'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbBrunch3day']) == 3 && !empty($data['getProduct2brunch']->use_id)){
            if($product->id == $data['carbBrunch3day'][$i]){
                echo round($data['carbsperservingUseEmptybrunch'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbBrunch3day']) == 4 && !empty($data['getProduct3brunch']->use_id)){
            if($product->id == $data['carbBrunch3day'][$i]){
                echo round($data['carbsperservingUseEmptybrunch'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['fatBrunch3day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['fatBrunch3day'][$i]){
            echo round($data['fatsperservingUseEmptybrunch']/2 * 100/ $product->fat).  " g". "<br>";
        }
    }else{

        if(count($data['fatBrunch3day']) == 2 && empty($data['getPro1brunch']->use_id)){
            if($product->id == $data['faLuncht'][$i]){
                echo round($data['fatsperservingUseEmptybrunch']  * 100/ $product->fat).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['fatBrunch3day']) == 2 && !empty($data['getPro1brunch']->use_id)){
            if($product->id == $data['fatBrunch3day'][$i]){
                echo round($data['fatsperservingUseEmptybrunch'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatBrunch3day']) == 3 && !empty($data['getPro1']->use_id) && !empty($data['getPro2brunch']->use_id)){
            if($product->id == $data['fatBrunch3day'][$i]){
                echo round($data['fatsperservingUseEmptybrunch'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatBrunch3day']) == 3 && !empty($data['getPro2brunch']->use_id)){
            if($product->id == $data['fatBrunch3day'][$i]){
                echo round($data['fatsperservingUseEmptybrunch'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatBrunch3day']) == 4 && !empty($data['getPro3brunch']->use_id)){
            if($product->id == $data['fatBrunch3day'][$i]){
                echo round($data['fatsperservingUseEmptybrunch'] * 100/ $product->fat).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['otherBrunch3day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['otherBrunch3day'][$i]){
            echo round($data['othersperservingUseEmptybrunch']/2 * 100/ $product->carb).  " g". "<br>";
        }
    }else{

        if(count($data['otherBrunch3day']) == 2 && empty($data['getPr1brunch']->use_id)){
            if($product->id == $data['otherBrunch3day'][$i]){
                echo round($data['othersperservingUseEmptybrunch']/2 * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['otherBrunch3day']) == 2 && !empty($data['getPr1brunch']->use_id)){
            if($product->id == $data['otherBrunch3day'][$i]){
                echo round($data['othersperservingUseEmptybrunch'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherBrunch3day']) == 3 && !empty($data['getPr1brunch']->use_id) && !empty($data['getPr2brunch']->use_id)){
            if($product->id == $data['otherBrunch3day'][$i]){
                echo round($data['othersperservingUseEmptybrunch'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherBrunch3day']) == 3 && !empty($data['getPr2brunch']->use_id)){
            if($product->id == $data['otherBrunchlunch'][$i]){
                echo round($data['othersperservingUseEmptybrunch']/2 * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherBrunch3day']) == 4 && !empty($data['getPr3brunch']->use_id)){
            if($product->id == $data['otherBrunch3day'][$i]){
                echo round($data['othersperservingUseEmptybrunch'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<h5>Recipe</h5>
<font size="2"><?php echo $data['brunch3Day']->recipe; ?></font>
</div>
</div>
</div>
    </div></div>
