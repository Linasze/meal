<div id="headingOne">
   ...
   <div class="col-lg-2 d-none d-lg-block text-center"> <?php echo date('h:i',strtotime($data['user_settings']->wake_up));?><br> Breakfast</div>
     ...
     <img src="<?php echo URLROOT;?>/public/img/arrow-down.svg" height="15" weight="15">
       <font size="2"><?php echo $data['breakfast3Day']->title; ?></font>
     </button>
   </div>
...
<div class="col-12">Breakfast<br>Time: <?php echo date('h:i',strtotime($data['user_settings']->wake_up));?></div>
    <div class="col-12"><?php echo $data['breakfast3Day']->title; ?></div>
...
<div class="col-12">Recipe: <input type="image" src="<?php echo URLROOT;?>/public/img/eye-solid.svg" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne" height="25" width="25"></div>
     ...
     <div class="col-md-12 d-none d-md-block d-lg-none">
        ...
        <div class="col-md-12"><?php echo $data['breakfast3Day']->title; ?></div>
        <hr>
...
<?php if($product->id == $data['protein3day'][$i]): ?>
<?php echo $product->title . " ";?>
<?php

if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['protein3day'][$i]){
...
<?php if($product->id == $data['carb3day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['carb3day'][$i]){
            echo round($data['carbsperservingUseEmpty']/2 * 100/ $product->carb).  " g". "<br>";
        }
    }else{

        if(count($data['carb3day']) == 2 && empty($data['getProduct1']->use_id)){
            if($product->id == $data['carb3day'][$i]){
                echo round($data['carbsperservingUseEmpty']  * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['carb3day']) == 2 && !empty($data['getProduct1']->use_id)){
            if($product->id == $data['carb3day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carb3day']) == 3 && !empty($data['getProduct1']->use_id) && !empty($data['getProduct2']->use_id)){
            if($product->id == $data['carb3day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carb3day']) == 3 && !empty($data['getProduct2']->use_id)){
            if($product->id == $data['carb3day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carb3day']) == 4 && !empty($data['getProduct3']->use_id)){
            if($product->id == $data['carb3day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['fat3day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['fat3day'][$i]){
            echo round($data['fatsperservingUseEmpty']/2 * 100/ $product->fat).  " g". "<br>";
        }
    }else{

        if(count($data['fat3day']) == 2 && empty($data['getPro1']->use_id)){
            if($product->id == $data['fat3day'][$i]){
                echo round($data['fatsperservingUseEmpty']  * 100/ $product->fat).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['fat3day']) == 2 && !empty($data['getPro1']->use_id)){
            if($product->id == $data['fat3day'][$i]){
                echo round($data['fatsperservingUseEmpty'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fat3day']) == 3 && !empty($data['getPro1']->use_id) && !empty($data['getPro2']->use_id)){
            if($product->id == $data['fat3day'][$i]){
                echo round($data['fatsperservingUseEmpty'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fat3day']) == 3 && !empty($data['getPro2']->use_id)){
            if($product->id == $data['fat3day'][$i]){
                echo round($data['fatsperservingUseEmpty'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fat3day']) == 4 && !empty($data['getPro3']->use_id)){
            if($product->id == $data['fat3day'][$i]){
                echo round($data['fatsperservingUseEmpty'] * 100/ $product->fat).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['other3day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['other3day'][$i]){
            echo round($data['othersperservingUseEmpty']/2 * 100/ $product->carb).  " g". "<br>";
        }
    }else{

        if(count($data['other3day']) == 2 && empty($data['getPr1']->use_id)){
            if($product->id == $data['other3day'][$i]){
                echo round($data['othersperservingUseEmpty']/2 * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['other3day']) == 2 && !empty($data['getPr1']->use_id)){
            if($product->id == $data['other3day'][$i]){
                echo round($data['othersperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['other3day']) == 3 && !empty($data['getPr1']->use_id) && !empty($data['getPr2']->use_id)){
            if($product->id == $data['other3day'][$i]){
                echo round($data['othersperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['other3day']) == 3 && !empty($data['getPr2']->use_id)){
            if($product->id == $data['other3day'][$i]){
                echo round($data['othersperservingUseEmpty']/2 * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['other3day']) == 4 && !empty($data['getPr3']->use_id)){
            if($product->id == $data['other3day'][$i]){
                echo round($data['othersperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<div class="col-md-8 col- text-left">
    <h5>Recipe</h5>
    <font size="2">
        <?php echo $data['breakfast3Day']->recipe; ?>
    </font>
</div>
</div>
</div>
