<div id="headingFour">
   <div class="card-header">
     ...
     <img src="<?php echo URLROOT;?>/public/img/arrow-down.svg" height="15" weight="15">
       <font size="2"><?php echo $data['afternoon3Day']->title; ?></font>
     </button>
   </div>
...
<div class="col-12">Afternoon meal<br>Time: <?php echo date('h:i',strtotime($data['user_settings']->wake_up)) + 9 . ":00";?></div>
    <div class="col-12"><?php echo $data['afternoon3Day']->title; ?></div>
...
<div class="col-12">Recipe: <input type="image" src="<?php echo URLROOT;?>/public/img/eye-solid.svg" data-toggle="collapse" data-target="#collapseFour" aria-expanded="true" aria-controls="collapseFour" height="25" width="25"></div>
     ...
     <div class="col-md-12 d-none d-md-block d-lg-none">
        ...
        <div class="col-md-12"><?php echo $data['afternoon3Day']->title; ?></div>
        <hr>
...
<?php if($product->id == $data['proteinAfternoon3day'][$i]): ?>
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['proteinAfternoon3day'][$i]){
            echo round($data['proteinsperservingUseEmptyafternoon']/2 * 100/ $product->protein).  " g". "<br>";
        } 
    }else{
 
        if(count($data['proteinAfternoon3day']) == 2 && empty($data['get_product1afternoon']->use_id)){
            if($product->id == $data['proteinAfternoon3day'][$i]){
                echo round($data['proteinsperservingUseEmptyafternoon']/2 * 100/ $product->protein).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['proteinAfternoon3day']) == 2 && !empty($data['get_product1afternoon']->use_id)){
            if($product->id == $data['proteinAfternoon3day'][$i]){
                echo round($data['proteinsperservingUseEmptyafternoon'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinAfternoon3day']) == 3 && !empty($data['get_product1afternoon']->use_id) && !empty($data['get_product2afternoon']->use_id)){
            if($product->id == $data['proteinAfternoon3day'][$i]){
                echo round($data['proteinsperservingUseEmptyafternoon'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinAfternoon3day']) == 3 && !empty($data['get_product2afternoon']->use_id)){
            if($product->id == $data['proteinAfternoon3day'][$i]){
                echo round($data['proteinsperservingUseEmptyafternoon']/2 * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinAfternoon3day']) == 4 && !empty($data['get_product3afternoon']->use_id)){
            if($product->id == $data['proteinAfternoon3day'][$i]){
                echo round($data['proteinsperservingUseEmptyafternoon'] * 100/ $product->protein).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['carbAfternoon3day'][$i]): ?>
<?php echo $product->title . " ";?> 
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['carbAfternoon3day'][$i]){
            echo round($data['carbsperservingUseEmptyafternoon']/2 * 100/ $product->carb).  " g". "<br>";
        } 
    }else{
 
        if(count($data['carbAfternoon3day']) == 2 && empty($data['getProduct1afternoon']->use_id)){
            if($product->id == $data['carbAfternoon3day'][$i]){
                echo round($data['carbsperservingUseEmptyafternoon']  * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['carbAfternoon3day']) == 2 && !empty($data['getProduct1afternoon']->use_id)){
            if($product->id == $data['carbAfternoon3day'][$i]){
                echo round($data['carbsperservingUseEmptyafternoon'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbAfternoon3day']) == 3 && !empty($data['getProduct1afternoon']->use_id) && !empty($data['getProduct2afternoon']->use_id)){
            if($product->id == $data['carbAfternoon3day'][$i]){
                echo round($data['carbsperservingUseEmptyafternoon'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbAfternoon3day']) == 3 && !empty($data['getProduct2afternoon']->use_id)){
            if($product->id == $data['carbAfternoon3day'][$i]){
                echo round($data['carbsperservingUseEmptyafternoon'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbAfternoon3day']) == 4 && !empty($data['getProduct3afternoon']->use_id)){
            if($product->id == $data['carbAfternoon3day'][$i]){
                echo round($data['carbsperservingUseEmptyafternoon'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['fatAfternoon3day'][$i]): ?>
<?php echo $product->title . " ";?> 
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['fatAfternoon3day'][$i]){
            echo round($data['fatsperservingUseEmptyafternoon']/2 * 100/ $product->fat).  " g". "<br>";
        } 
    }else{
 
        if(count($data['fatAfternoon3day']) == 2 && empty($data['getPro1afternoon']->use_id)){
            if($product->id == $data['faLuncht'][$i]){
                echo round($data['fatsperservingUseEmptyafternoon']  * 100/ $product->fat).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['fatAfternoon3day']) == 2 && !empty($data['getPro1afternoon']->use_id)){
            if($product->id == $data['fatAfternoon3day'][$i]){
                echo round($data['fatsperservingUseEmptyafternoon'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatAfternoon3day']) == 3 && !empty($data['getPro1']->use_id) && !empty($data['getPro2afternoon']->use_id)){
            if($product->id == $data['fatAfternoon3day'][$i]){
                echo round($data['fatsperservingUseEmptyafternoon'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatAfternoon3day']) == 3 && !empty($data['getPro2afternoon']->use_id)){
            if($product->id == $data['fatAfternoon3day'][$i]){
                echo round($data['fatsperservingUseEmptyafternoon'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatAfternoon3day']) == 4 && !empty($data['getPro3afternoon']->use_id)){
            if($product->id == $data['fatAfternoon3day'][$i]){
                echo round($data['fatsperservingUseEmptyafternoon'] * 100/ $product->fat).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['otherAfternoon3day'][$i]): ?>
<?php echo $product->title . " ";?> 
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['otherAfternoon3day'][$i]){
            echo round($data['othersperservingUseEmptyafternoon']/2 * 100/ $product->carb).  " g". "<br>";
        } 
    }else{
 
        if(count($data['otherAfternoon3day']) == 2 && empty($data['getPr1afternoon']->use_id)){
            if($product->id == $data['otherAfternoon3day'][$i]){
                echo round($data['othersperservingUseEmptyafternoon']/2 * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['otherAfternoon3day']) == 2 && !empty($data['getPr1afternoon']->use_id)){
            if($product->id == $data['otherAfternoon3day'][$i]){
                echo round($data['othersperservingUseEmptyafternoon'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherAfternoon3day']) == 3 && !empty($data['getPr1afternoon']->use_id) && !empty($data['getPr2afternoon']->use_id)){
            if($product->id == $data['otherAfternoon3day'][$i]){
                echo round($data['othersperservingUseEmptyafternoon'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherAfternoon3day']) == 3 && !empty($data['getPr2afternoon']->use_id)){
            if($product->id == $data['otherAfternoonlunch'][$i]){
                echo round($data['othersperservingUseEmptyafternoon']/2 * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherAfternoon3day']) == 4 && !empty($data['getPr3afternoon']->use_id)){
            if($product->id == $data['otherAfternoon3day'][$i]){
                echo round($data['othersperservingUseEmptyafternoon'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<div class="col-md-8 col- text-left">
    <h5>Recipe</h5>
    <font size="2">
        <?php echo $data['afternoon3Day']->recipe; ?>
    </font>
</div>
</div>
</div>
