...
<div class="col-12">Breakfast<br>Time: <?php echo date('h:i',strtotime($data['user_settings']->wake_up));?></div>
    <div class="col-12"><?php echo $data['breakfast6Day']->title; ?></div>
...
<div class="col-12">Recipe: <input type="image" src="<?php echo URLROOT;?>/public/img/eye-solid.svg" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne" height="25" width="25"></div>
     ...
     <div class="col-md-12 d-none d-md-block d-lg-none">
        ...
        <div class="col-md-12"><?php echo $data['breakfast6Day']->title; ?></div>
        <hr>
...
<?php if($product->id == $data['protein6day'][$i]): ?>
<?php echo $product->title . " ";?>
<?php

if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['protein6day'][$i]){
            echo round($data['proteinsperservingUseEmpty']/2 * 100/ $product->protein).  " g". "<br>";
        }
    }else{

        if(count($data['protein6day']) == 2 && empty($data['get_product1']->use_id)){
            if($product->id == $data['protein6day'][$i]){
                echo round($data['proteinsperservingUseEmpty']/2 * 100/ $product->protein).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['protein6day']) == 2 && !empty($data['get_product1']->use_id)){
            if($product->id == $data['protein6day'][$i]){
                echo round($data['proteinsperservingUseEmpty'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['protein6day']) == 3 && !empty($data['get_product1']->use_id) && !empty($data['get_product2']->use_id)){
            if($product->id == $data['protein6day'][$i]){
                echo round($data['proteinsperservingUseEmpty'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['protein6day']) == 3 && !empty($data['get_product2']->use_id)){
            if($product->id == $data['protein6day'][$i]){
                echo round($data['proteinsperservingUseEmpty']/2 * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['protein6day']) == 4 && !empty($data['get_product3']->use_id)){
            if($product->id == $data['protein6day'][$i]){
                echo round($data['proteinsperservingUseEmpty'] * 100/ $product->protein).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['carb6day'][$i]): ?>
<?php echo $product->title . " ";?>
...
<?php if($product->id == $data['fat6day'][$i]): ?>
<?php echo $product->title . " ";?>
...
<?php if($product->id == $data['other6day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['other6day'][$i]){
