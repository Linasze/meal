<div id="headingFifth">
    ...
    <div class="card-header">
     ...
     <button class="btn btn-link text-secondary float-left" data-toggle="collapse" data-target="#collapseFifth" aria-expanded="true" aria-controls="collapseFifth">
       ...
       <font size="2"><?php echo $data['dinner2Day']->title; ?></font>
     </button>
     </div>
...
<div class="col-12">Dinner<br>Time: <?php echo date('h:i',strtotime($data['user_settings']->wake_up)) + 12 . ":00";?></div>
    <div class="col-12"><?php echo $data['dinner2Day']->title; ?></div>
...
<div class="col-12">Recipe: <input type="image" src="<?php echo URLROOT;?>/public/img/eye-solid.svg" data-toggle="collapse" data-target="#collapseFifth" aria-expanded="true" aria-controls="collapseFifth" height="25" width="25"></div>
     ...
     <div class="col-md-12 d-none d-md-block d-lg-none">
        ...
        <div class="col-md-12"><?php echo $data['dinner2Day']->title; ?></div>
        <hr>
...
<?php if($product->id == $data['proteinDinner2day'][$i]): ?>
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['proteinDinner2day'][$i]){
            echo round($data['proteinsperservingUseEmptydinner']/2 * 100/ $product->protein).  " g". "<br>";
        } 
    }else{
 
        if(count($data['proteinDinner2day']) == 2 && empty($data['get_product1dinner']->use_id)){
            if($product->id == $data['proteinDinner2day'][$i]){
                echo round($data['proteinsperservingUseEmptydinner']/2 * 100/ $product->protein).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['proteinDinner2day']) == 2 && !empty($data['get_product1dinner']->use_id)){
            if($product->id == $data['proteinDinner2day'][$i]){
                echo round($data['proteinsperservingUseEmptydinner'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinDinner2day']) == 3 && !empty($data['get_product1dinner']->use_id) && !empty($data['get_product2dinner']->use_id)){
            if($product->id == $data['proteinDinner2day'][$i]){
                echo round($data['proteinsperservingUseEmptydinner'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinDinner2day']) == 3 && !empty($data['get_product2dinner']->use_id)){
            if($product->id == $data['proteinDinner2day'][$i]){
                echo round($data['proteinsperservingUseEmptydinner']/2 * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['proteinDinner2day']) == 4 && !empty($data['get_product3dinner']->use_id)){
            if($product->id == $data['proteinDinner2day'][$i]){
                echo round($data['proteinsperservingUseEmptydinner'] * 100/ $product->protein).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['carbDinner2day'][$i]): ?>
<?php echo $product->title . " ";?> 
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['carbDinner2day'][$i]){
            echo round($data['carbsperservingUseEmptydinner']/2 * 100/ $product->carb).  " g". "<br>";
        } 
    }else{
 
        if(count($data['carbDinner2day']) == 2 && empty($data['getProduct1dinner']->use_id)){
            if($product->id == $data['carbDinner2day'][$i]){
                echo round($data['carbsperservingUseEmptydinner']  * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['carbDinner2day']) == 2 && !empty($data['getProduct1dinner']->use_id)){
            if($product->id == $data['carbDinner2day'][$i]){
                echo round($data['carbsperservingUseEmptydinner'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbDinner2day']) == 3 && !empty($data['getProduct1dinner']->use_id) && !empty($data['getProduct2dinner']->use_id)){
            if($product->id == $data['carbDinner2day'][$i]){
                echo round($data['carbsperservingUseEmptydinner'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbDinner2day']) == 3 && !empty($data['getProduct2dinner']->use_id)){
            if($product->id == $data['carbDinner2day'][$i]){
                echo round($data['carbsperservingUseEmptydinner'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carbDinner2day']) == 4 && !empty($data['getProduct3dinner']->use_id)){
            if($product->id == $data['carbDinner2day'][$i]){
                echo round($data['carbsperservingUseEmptydinner'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['fatDinner2day'][$i]): ?>
<?php echo $product->title . " ";?> 
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['fatDinner2day'][$i]){
            echo round($data['fatsperservingUseEmptydinner']/2 * 100/ $product->fat).  " g". "<br>";
        } 
    }else{
 
        if(count($data['fatDinner2day']) == 2 && empty($data['getPro1dinner']->use_id)){
            if($product->id == $data['faLuncht'][$i]){
                echo round($data['fatsperservingUseEmptydinner']  * 100/ $product->fat).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['fatDinner2day']) == 2 && !empty($data['getPro1dinner']->use_id)){
            if($product->id == $data['fatDinner2day'][$i]){
                echo round($data['fatsperservingUseEmptydinner'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatDinner2day']) == 3 && !empty($data['getPro1']->use_id) && !empty($data['getPro2dinner']->use_id)){
            if($product->id == $data['fatDinner2day'][$i]){
                echo round($data['fatsperservingUseEmptydinner'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatDinner2day']) == 3 && !empty($data['getPro2dinner']->use_id)){
            if($product->id == $data['fatDinner2day'][$i]){
                echo round($data['fatsperservingUseEmptydinner'] * 100/ $product->fat).  " g". "<br>";
            }
        }elseif(count($data['fatDinner2day']) == 4 && !empty($data['getPro3dinner']->use_id)){
            if($product->id == $data['fatDinner2day'][$i]){
                echo round($data['fatsperservingUseEmptydinner'] * 100/ $product->fat).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['otherDinner2day'][$i]): ?>
<?php echo $product->title . " ";?> 
...
if(empty($product->use_id)){ 
    if(!empty($product->use_id)){
        if($product->id == $data['otherDinner2day'][$i]){
            echo round($data['othersperservingUseEmptydinner']/2 * 100/ $product->carb).  " g". "<br>";
        } 
    }else{
 
        if(count($data['otherDinner2day']) == 2 && empty($data['getPr1dinner']->use_id)){
            if($product->id == $data['otherDinner2day'][$i]){
                echo round($data['othersperservingUseEmptydinner']/2 * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is  
        }elseif(count($data['otherDinner2day']) == 2 && !empty($data['getPr1dinner']->use_id)){
            if($product->id == $data['otherDinner2day'][$i]){
                echo round($data['othersperservingUseEmptydinner'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherDinner2day']) == 3 && !empty($data['getPr1dinner']->use_id) && !empty($data['getPr2dinner']->use_id)){
            if($product->id == $data['otherDinner2day'][$i]){
                echo round($data['othersperservingUseEmptydinner'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherDinner2day']) == 3 && !empty($data['getPr2dinner']->use_id)){
            if($product->id == $data['otherDinnerlunch'][$i]){
                echo round($data['othersperservingUseEmptydinner']/2 * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['otherDinner2day']) == 4 && !empty($data['getPr3dinner']->use_id)){
            if($product->id == $data['otherDinner2day'][$i]){
                echo round($data['othersperservingUseEmptydinner'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<div class="col-md-8 col- text-left">
    <h5>Recipe</h5>
    <font size="2">
        <?php echo $data['dinner2Day']->recipe; ?>
    </font>
</div>
</div>
</div>
