...
<?php if($product->id == $data['protein7day'][$i]): ?>
<?php echo $product->title . " ";?>
<?php

if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['protein7day'][$i]){
            echo round($data['proteinsperservingUseEmpty']/2 * 100/ $product->protein).  " g". "<br>";
        }
    }else{

        if(count($data['protein7day']) == 2 && empty($data['get_product1']->use_id)){
            if($product->id == $data['protein7day'][$i]){
                echo round($data['proteinsperservingUseEmpty']/2 * 100/ $product->protein).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['protein7day']) == 2 && !empty($data['get_product1']->use_id)){
            if($product->id == $data['protein7day'][$i]){
                echo round($data['proteinsperservingUseEmpty'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['protein7day']) == 3 && !empty($data['get_product1']->use_id) && !empty($data['get_product2']->use_id)){
            if($product->id == $data['protein7day'][$i]){
                echo round($data['proteinsperservingUseEmpty'] * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['protein7day']) == 3 && !empty($data['get_product2']->use_id)){
            if($product->id == $data['protein7day'][$i]){
                echo round($data['proteinsperservingUseEmpty']/2 * 100/ $product->protein).  " g". "<br>";
            }
        }elseif(count($data['protein7day']) == 4 && !empty($data['get_product3']->use_id)){
            if($product->id == $data['protein7day'][$i]){
                echo round($data['proteinsperservingUseEmpty'] * 100/ $product->protein).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['carb7day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['carb7day'][$i]){
            echo round($data['carbsperservingUseEmpty']/2 * 100/ $product->carb).  " g". "<br>";
        }
    }else{

        if(count($data['carb7day']) == 2 && empty($data['getProduct1']->use_id)){
            if($product->id == $data['carb7day'][$i]){
                echo round($data['carbsperservingUseEmpty']  * 100/ $product->carb).  " g". "<br>";
            }
            // Two products one with out use_id and one with use_is
        }elseif(count($data['carb7day']) == 2 && !empty($data['getProduct1']->use_id)){
            if($product->id == $data['carb7day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carb7day']) == 3 && !empty($data['getProduct1']->use_id) && !empty($data['getProduct2']->use_id)){
            if($product->id == $data['carb7day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carb7day']) == 3 && !empty($data['getProduct2']->use_id)){
            if($product->id == $data['carb7day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }elseif(count($data['carb7day']) == 4 && !empty($data['getProduct3']->use_id)){
            if($product->id == $data['carb7day'][$i]){
                echo round($data['carbsperservingUseEmpty'] * 100/ $product->carb).  " g". "<br>";
            }
        }else{
...
<?php if($product->id == $data['fat7day'][$i]): ?>
<?php echo $product->title . " ";?>
...
<?php if($product->id == $data['other7day'][$i]): ?>
<?php echo $product->title . " ";?>
...
if(empty($product->use_id)){
    if(!empty($product->use_id)){
        if($product->id == $data['other7day'][$i]){
